...
class Track:
    ...
    def __init__(self, detection, track_id, n_init, max_age, feature=None, score=None):
        ...
        self.state = TrackState.Tentative  # 初始状态
        self.features = []
        ...
        self.confidence = score  # 存储匹配上后的置信度
    ...
    def update(self, detection):
        ...
        # TODO EMA特征更新方式, 每个轨迹只存一个特征
        if opt.EMA:
            smooth_feat = opt.EMA_alpha * \
                          self.features[-1] + (1 - opt.EMA_alpha) * feature
            smooth_feat /= np.linalg.norm(smooth_feat)  # 再次归一化
